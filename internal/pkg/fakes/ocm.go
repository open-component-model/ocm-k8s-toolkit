...
func (c *Context) RepositoryForConfig(_ []byte, _ ocmruntime.Unmarshaler, _ ...credentials.CredentialsSource) (ocm.Repository, error) {
	return c.repo, nil
}
...
func (m *mockComponentAccess) LookupVersion(version string) (ocm.ComponentVersionAccess, error) {
	for _, c := range m.context.components[m.name] {
