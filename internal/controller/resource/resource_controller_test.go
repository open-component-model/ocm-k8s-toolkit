	...
	Context("component controller", func() {
		...
		Context("resource controller", func() {
			It("can reconcile a resource: PlainText", func() {
				...
				componentObj = test.SetupComponentWithDescriptorList(ctx, testComponent, Namespace, dataCds, &test.MockComponentOptions{
					Registry: registry,
					Client:   k8sClient,
					Recorder: recorder,
				...
				Expect(err).NotTo(HaveOccurred())
				validateArtifact(ctx, resource, resourceAcc.Meta().Digest.Value)
...
func validateArtifact(ctx context.Context, resource *v1alpha1.Resource, digest string) {
	GinkgoHelper()
	...
	content := Must(ociRepo.FetchArtifact(ctx, resource.GetManifestDigest()))

	Expect(string(content)).To(Equal(ResourceContent))

	Expect(resource.GetBlobDigest()).To(Equal(digest))
	Expect(resource.Status.OCIArtifact.Blob.Tag).To(Equal(ResourceVersion))
	Expect(resource.Status.OCIArtifact.Blob.Size).To(Equal(int64(len(content))))
}
